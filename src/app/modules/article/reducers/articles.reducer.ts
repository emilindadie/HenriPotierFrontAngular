...
export function ArticlesListReducer(state: IArticlesListState = initialState, action: IBaseAction<IArticleListAction>): IArticlesListState {
    switch (action.type) {
        case 'LOAD_BOOKS':
        return {
            ...state,
        ...
        return {
            ...
            articlesItemsCount: state.articles.length + action.payload.articles.length,
            filteredArticlesItemsCount: state.articles.length + action.payload.articles.length,
        ...
        return {
            ...state,
            filteredArticles: state.articles.filter(p => p.title.includes(action.payload.searchPattern))
        };
        default:
        return state;
    }
